    ...
    def FER_live_cam():
        ...
        for (x1, y1, x2, y2) in boxes:
            ...
            if output is None or len(output) == 0:
                print("Ошибка: выходные данные модели пусты.")
                return "unknown"
            else:
                return emotion_dict[list(output[0]).index(max(output[0]))]
    preds = FER_live_cam()
    return preds
# frame = cv2.imread('D:/sobez/video_for_data/image.png')
# print(emotion_pred(frame))